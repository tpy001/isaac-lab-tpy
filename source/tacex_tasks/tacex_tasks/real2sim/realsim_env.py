...
class RealSimEnv(ForgeEnv):
    ...
    def _setup_scene(self):
        ...
        env_cfg = sim_utils.UsdFileCfg(
            usd_path="/home/zhuchengyang/isaac_lab_tpy/assets/franka_env/franka_env.usd"
        )

        env_cfg.func(
            "/World/envs/env_.*/franka_env",
            env_cfg,
            translation=(0.0, 0.0, 0.0),       # 先用(0,0,0)，看位置对不对再调
    ...
    def randomize_initial_state(self, env_ids):
        ...
        while True:
            ...
            print(f"[DEBUG] fingertip_pos  (current)[0]: {fingertip_pos_cur[0].cpu().numpy()}")
            print(f"[DEBUG] fingertip_quat (current)[0]: {fingertip_quat_cur[0].cpu().numpy()}")

            # ✅ 计算目标位置
            above_fixed_pos = fixed_tip_pos.clone()
            above_fixed_pos[:, 2] += self.cfg_task.hand_init_pos[2]
            print(f"[DEBUG] fixed_tip_pos          [0]: {fixed_tip_pos[0].cpu().numpy()}")
            print(f"[DEBUG] hand_init_pos z offset    : {self.cfg_task.hand_init_pos[2]}")
            print(f"[DEBUG] above_fixed_pos (no noise)[0]: {above_fixed_pos[0].cpu().numpy()}")
            ...
            print(f"[DEBUG] above_fixed_pos (w/ noise)[0]: {above_fixed_pos[0].cpu().numpy()}")

            # ✅ 打印目标和当前fingertip的差值
            delta = above_fixed_pos - fingertip_pos_cur
            print(f"[DEBUG] pos delta (target - current)[0]: {delta[0].cpu().numpy()}")
            print(f"[DEBUG] pos delta norm[0]: {torch.linalg.norm(delta[0]):.4f} m")
            ...
            print(f"[DEBUG] hand_down_euler (w/ noise)[0]: {hand_down_euler[0].cpu().numpy()}")

            hand_down_quat[bad_envs, :] = torch_utils.quat_from_euler_xyz(
                roll=hand_down_euler[:, 0], pitch=hand_down_euler[:, 1], yaw=hand_down_euler[:, 2]
            )
            print(f"[DEBUG] hand_down_quat (target)  [0]: {hand_down_quat[0].cpu().numpy()}")
            ...
            print(f"[DEBUG] pos_err   — max: {pos_err_val.max():.4f} | mean: {pos_err_val.mean():.4f}")
            print(f"[DEBUG] angle_err — max: {angle_err_val.max():.4f} | mean: {angle_err_val.mean():.4f}")
            ...
            print(f"[DEBUG] 收敛后仍失败的env数: {bad_envs.shape[0]}")

            if bad_envs.shape[0] == 0:
                print(f"[DEBUG] ✅ IK 全部收敛，共尝试 {ik_attempt + 1} 次")
                break
