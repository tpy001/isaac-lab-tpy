...
from .factory_tasks_cfg import FactoryTask, GearMesh, NutThread, PegInsert,ASSET_DIR
...
class FactoryEnvCfg(DirectRLEnvCfg):
    """
    Base configuration for Factory Environment (Without GelSight Sensors).
    """
    seed = 42
    decimation = 4 # if decimation is set to 8, which means update image every 8 physic steps. (This is used to save computation resource and increase realtime speed)
    # decimation = 8 # if decimation is set to 8, which means update image every 8 physic steps. (This is used to save computation resource and increase realtime speed)
    action_space = 6
    ...
    sim: SimulationCfg = SimulationCfg(
        ...
        render_interval = 4 # it is recommended to set as equal to decimation
    )
    ...
    policy_cfg = None
    # policy_cfg = PI0RemoteConfig()
    # policy_cfg = PI0RemoteTAVLAConfig(num_history_steps=10,history_step_interval=2)


  
    
@configclass
class FactoryEnvGelSightCfg(FactoryEnvCfg):
    """
    Configuration extending FactoryEnvCfg to include GelSight Mini sensors.
    """
    # Prepare marker config helper
    ...
    gsmini_left = GelSightMiniCfg(
        ...
        marker_motion_sim_cfg=FOTSMarkerSimulatorCfg(
            ...
            frame_transformer_cfg=FrameTransformerCfg(
                ...
                # Note: target_frames typically depends on the task (Peg, Gear, etc.)
                # Here we use the default peg asset from the snippet.
                target_frames=[FrameTransformerCfg.FrameCfg(prim_path="/World/envs/env_.*/HeldAsset/forge_round_peg_8mm")],
                debug_vis=False,
    ...
    # Define Right Sensor (inheriting from Left logic)
